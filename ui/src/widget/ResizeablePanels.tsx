...
export const ResizeablePanels:React.FC<ResizeablePanelsProps>  = ({children}) => {
    ...
    return (
        ...
        >
            <LeftPanel leftWidth={leftWidth} setLeftWidth={setLeftWidth} startingWidth={startWidth} minWidth={minWidth}>
                {left}
            </LeftPanel>
            <Divider size="xl" ref={dividerRef} orientation="vertical" onMouseDown={onMouseDown} onMouseUp={onMouseUp}
            style={{cursor:"grab", padding:"0 6px 0 6px"}}
            />
            <div ref={rightPanelRef} style={{flex: 1, height: "100%", position: "relative", minWidth: "14rem"}}>
                {right}
            </div>
        </Flex>
