...
class APIBridge {
    ...
    async list_books(stripped = true): Promise<Book[]> {
        return this.boundary.remote('list_books', stripped)
    }

    async get_current_book(stripped = true): Promise<Book | undefined> {
    ...
    async create_scene(chapter_id: UniqueId, title: string, position = -1) {
    ...
    async create_scene_status(
        book_uid: UniqueId,
        name: string,
        color: string,
        scene_uid: UniqueId | undefined = undefined
    ): Promise<SceneStatus> {
